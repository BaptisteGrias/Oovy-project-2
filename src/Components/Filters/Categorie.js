...
const handlefilterFilmByCategorie = (event, genre_id) => {
  ...
  const moviesByGenre =  films
          .filter((film) =>  film.genre_ids.includes(genre_id))
  ...
  return (
    <div>
      <div className="boutonCategorie">
        {categories
          .map((categorie, index) => (
            <CategorieButton key={index} handlefilterFilmByCategorie={handlefilterFilmByCategorie} genre_id={categorie.id} genre_name={categorie.name} />
          ))
          }
